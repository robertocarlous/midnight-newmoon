...
**Whisper Wall** is an anonymous feedback board for teams and communities:
anyone can post a message that becomes fully public and permanent, but
nobody — not even the contract, not even someone reading the chain — can
tell who posted it. Existing "anonymous" feedback tools (Google Forms,
Slack polls) are anonymous only because the *provider* chooses not to log
who submitted what; the operator could always de-anonymize you if they
wanted to, and you have to trust that they won't. On Midnight, the
author's identity is a private witness that never leaves the poster's own
machine — it is architecturally impossible for the contract, the indexer,
or anyone else to recover it, not just contractually promised. Level 1
ships the simplest version of this idea: post a message, read the wall,
and prove-without-revealing that the same wallet posted it twice via a
public commitment hash. Later cycles can build on this foundation with
per-author reply threads, moderation via disclosed proofs (e.g. "this
author is not on the banned list" without revealing which author), and
reactions that stay similarly unlinkable.

## Public state vs. private witness

The contract, [`contracts/whisper-wall.compact`](contracts/whisper-wall.compact),
deliberately splits data into two categories:
...
// Private witness - supplied locally by the TS driver, never touches
// the chain unless a circuit explicitly discloses it
...
- **Public ledger state** (`export ledger ...`) is committed to the chain
  on every transaction and readable by anyone via the indexer — no
  wallet, no permission needed. `feedbackCount` and `lastMessage` live
  here because being public is the entire point of a message board.
- **Private witness** (`witness authorSecret(): Bytes<32>`) is a function
  implemented in TypeScript ([`src/witnesses.ts`](src/witnesses.ts)), not
  in the contract. It runs locally inside the prover when a circuit needs
  it, and its return value is never sent anywhere by default.
- **`disclose()`** is the explicit, unmissable boundary between the two.
  Compact refuses to assign a value derived from a witness into ledger
  state unless it's wrapped in `disclose()` — you cannot leak private
  data by accident. Here, `disclose()` is used twice, for two different
  reasons:
  - `disclose(message)` — the message is supposed to be public; this is
    the entire product.
  - `disclose(commitment)` — the *hash* of the private secret is
...
| Compile output (circuits listed) | Contract deployed (address shown) |
| --- | --- |
...
2. `npm run compile` — compiles `contracts/whisper-wall.compact` to `contracts/managed/whisper-wall/`.
...
| `npm run test:e2e`      | Smoke + read-back check against the deployed contract (the test suite). |
| `npm run demo`          | Non-interactive: post one message, print the wall state back. |
...
midnight-newmoon/
├── contracts/
│   └── whisper-wall.compact    # Compact source
├── scripts/
│   ├── e2e-check.ts            # smoke + read-back + ledger-shape assertions
│   └── demo.ts                 # non-interactive: post one message, read the wall back
├── src/
│   ├── network.ts              # network selection + state file management
│   ├── wallet.ts               # wallet construction + sync-state cache
│   ├── witnesses.ts            # private witness implementation (authorSecret)
