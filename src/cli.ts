/**
 * CLI for interacting with midnight-newmoon contract
 */
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║                   midnight-newmoon CLI                           ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    while (running) {
      console.log('─── Menu ───────────────────────────────────────────────────────');
      console.log('  1. Post a message to the wall');
      console.log('  2. Read the wall (message count, last message, author commitment)');
      ...
      switch (choice.trim()) {
        ...
        case '2': {
          console.log('\n  Reading the wall from the blockchain...');
          try {
            const contractState = await providers.publicDataProvider.queryContractState(deployment.address);
            if (contractState) {
              const ledgerState = WhisperWall.ledger(contractState.data);
              const commitment = Buffer.from(ledgerState.lastAuthorCommitment).toString('hex');
              console.log(`\n  📋 Message count: ${ledgerState.feedbackCount}`);
              console.log(`  📋 Last message: "${ledgerState.lastMessage}"`);
              console.log(`  📋 Last author commitment (hash, not identity): ${commitment}\n`);
            } else {
